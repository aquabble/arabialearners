export const config = { runtime: "nodejs" };
  ...
  const { loadGlossary, extractLexicon, makeTemplateSentences, postJSON } = require("./_lib.cjs");
  ...
  export default async function handler(req, res) {
    ...
    try {
      ...
      if (!data && typeof fetch === "function") {
        try {
          const r = await fetch(new URL("/Glossary.json", url.origin), { cache: "no-store" });
      ...
      if (!items || !items.length) items = makeTemplateSentences(lex, { size, direction, difficulty });
