...
- Prefer provided LEX terms; avoid rare words.`;
  const user = JSON.stringify({ size: Math.max(1, Math.min(50, Number(size||5))), direction: direction||"ar2en", difficulty: difficulty||"medium", lex: sampleLex });
  try {
    const r = await postJSON(`${baseURL}/chat/completions`, {
      model, temperature: temp, response_format: { type: "json_object" },
    ...
    const items = arr.map((it) => {
      ...
      return (direction === "en2ar") ? { prompt: en, answer: ar, ar, en } : { prompt: ar, answer: en, ar, en };
    });
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!data && typeof fetch === "function") {
      try { const r = await fetch(new URL("/Glossary.json", url.origin), { cache: "no-store" }); if (r.ok) data = await r.json(); } catch {}
    }
    ...
    res.status(200).json({ ok:true, meta:{ size, difficulty, direction, llm: !!(items && process.env.OPENAI_API_KEY) }, items });
  } catch (e) {
