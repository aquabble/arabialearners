...
export default async function handler(req) {
  ...
  if (action === "size") {
    const size = await bufferSize({ semester, unit, chapter });
    return json({ ok: true, size });
  }

  if (action === "pop") {
    const item = await bufferPop({ semester, unit, chapter });
    return json({ ok: true, item });
  }

  if (action === "putOne") {
    const body = await req.json().catch(()=> ({}));
    const ok = await bufferPut({ ar: body.ar, en: body.en, semester, unit, chapter });
